...
void DisplayThreadArgs(ThreadArgs*	myThreadArg)
{
	...
	if( myThreadArg )
	{
			...
			printf("    MAX_THREAD_COUNT    TimeStamp [%"PRId64"]\n", myThreadArg->timeStamp[0] );
...
{ //TODO
   ...
   sigaddset(&(info->timer_signal), nextSigNum);
   ...
   mySignalEvent.sigev_signo = info->signalValue;
   mySignalEvent.sigev_value.sival_ptr = (void *)&(info->timerId);
   ret = timer_create(CLOCK_MONOTONIC, &mySignalEvent, &info->timerId);

   //Arm Timer – See code in background section ****

   struct itimerspecMAX_THREAD_COUNT timerSpec;

   seconds = period/1000000;
   nanoseconds = (period - (seconds * 100000)) * 1000;
...
static void wait_period (ThreadArgs *info)
{
  //Use sigwait function to wait on the timer_signal
	sigwait(&(timer_signal));
  //Update missed_signal_count by calling "timer_getoverrun"
  timer_getoverrun(missed_signal_count);
}
